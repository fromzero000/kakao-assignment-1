...
- 해결 방법 : 
    1. 상태 관리를 통해 `App.jsx`에서 `filter` 상태를 정의하고, 렌더링 전에 자바스크립트 `Array.filter()`를 사용해 가공된 `todos` 배열만 하위 컴포넌트에 전달하는 완전한 Data-Driven 방식으로 전환하여 해결했다.
...
- 아쉬운 점 : 아직 컴포넌트 간 상태 공유(Props Drilling)가 많아져서 관리가 복잡해지는 느낌을 받았습니다.
